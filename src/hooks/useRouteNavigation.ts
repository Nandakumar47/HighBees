...
export const useRouteNavigation = () => {
  ...
  const navigateToDestination = useCallback((destinationSlug: string) => {
  ...
  const navigateToEnquiry = useCallback((destinationSlug?: string) => {
  ...
  const isDestinationRoute = useCallback((pathname?: string) => {
    const currentPath = pathname || location.pathname;
    return currentPath.startsWith('/destinations/');
  }, [location.pathname]);

  const isDestinationsRoute = useCallback((pathname?: string) => {
    const currentPath = pathname || location.pathname;
    return currentPath === '/destinations' || currentPath.startsWith('/destinations/');
  }, [location.pathname]);
